...
class PrimaryMasterConfig(OnecloudConfig):
    ...
    def __init__(self, config, bastion_host=None):
        ...
        # Detect IP type first for all subsequent configurations
...
class WorkerConfig(OnecloudJointConfig):
    ...
    def ansible_vars(self):
        ...
        if self.primary_master_config:
            ...
            vars[KEY_K3S_API_ENDPOINT] = pc.controlplane_host
            vars[KEY_K3S_API_PORT] = pc.controlplane_port
        else:
            # Minimal fallback for AddNodesConfig (when primary_master_config is None)
            # Only set essential variables to prevent template failures
            match_ip, ip_type = match_ipaddr(self.controlplane_host)
            vars['ip_type'] = ip_type if match_ip else consts.IP_TYPE_IPV4
            vars[KEY_K3S_API_ENDPOINT] = self.controlplane_host
            vars[KEY_K3S_API_PORT] = self.controlplane_port
            if ip_type == consts.IP_TYPE_IPV6:
                vars['ip_autodetection_method'] = "'first-found'"
            else:
                vars['ip_autodetection_method'] = "'can-reach=%s'" % self.controlplane_host
